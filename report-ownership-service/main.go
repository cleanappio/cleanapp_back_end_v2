...
func main() {
	...
	if err != nil { log.Fatal("Failed to setup database:", err) }
	defer db.Close()
	if cfg.RunDBMigrations {
		log.Printf("Runtime DB migrations are disabled at service boot. Run report-ownership-service/cmd/migrate instead.")
	}
	ownershipService := database.NewOwnershipService(db)
	svc, err := service.NewService(cfg, ownershipService)
	if err != nil { log.Fatal("Failed to create service:", err) }
	if err := svc.Start(); err != nil { log.Fatal("Failed to start service:", err) }
	router := setupRouter(svc)
	srv := serverx.New(":8082", router)
	go func() {
		log.Printf("Starting HTTP server on port 8082")
		if err := srv.ListenAndServe(); err != nil && err != http.ErrServerClosed { log.Fatal("Failed to start HTTP server:", err) }
	}()
	...
	defer cancel()
	if err := svc.Stop(); err != nil { log.Printf("Error stopping service: %v", err) }
...
func setupDatabase(cfg *config.Config) (*sql.DB, error) {
	...
	for {
		...
		if time.Now().After(deadline) { return nil, fmt.Errorf("database ping timeout") }
		log.Printf("Database connection failed, retrying in %v", waitInterval)
		time.Sleep(waitInterval)
		if waitInterval < 4*time.Second { waitInterval *= 2 }
	}
