...
# VM-local golden path remediation:
# If a report is "stuck" (older than MIN_AGE_MIN, no report_analysis row),
# republish its seq to report.raw and wait for analysis.
#
# Defaults are chosen to avoid churn and avoid creating new work when the
