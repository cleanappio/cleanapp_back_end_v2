...
const (
	maxImageHeight  = 1000 // Maximum height in pixels
	minImageQuality = 85   // Minimum JPEG quality
)
...
// CompressImage compresses a JPEG image to have max height of 1000 pixels
...
func CompressImage(imageData []byte) ([]byte, error) {
	...
	if orientation != 1 {
		...
		log.Printf("Applied orientation correction: %d", orientation)
	}
	...
	if originalHeight <= maxImageHeight {
		// Image is already within height limits, return as-is
		return imageData, nil
	}

	// Calculate scale to fit within maxImageHeight while preserving aspect ratio
	scale := float64(maxImageHeight) / float64(originalHeight)

	// Calculate new dimensions
	newWidth := int(float64(originalWidth) * scale)
	newHeight := int(float64(originalHeight) * scale)

	// Ensure height doesn't exceed the limit
	...
	log.Printf("Image compressed: %d bytes -> %d bytes (quality: %d, scale: %.2f, original: %dx%d, new: %dx%d, orientation: %d)",
