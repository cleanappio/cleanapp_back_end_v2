...
pub async fn create_pool(config: &crate::config::Config) -> Result<Pool<MySql>> {
    ...
    log::info!(
        "Database URL constructed (password hidden): mysql://{}:***@{}:{}/{}",
        config.db_user,
        config.db_host,
        config.db_port,
        config.db_name
        ...
        match pool_options.connect(&database_url).await {
            Ok(pool) => {
                ...
                match test_connection_with_retries(&pool, 3).await {
                    ...
                    Err(e) => {
                        log::warn!(
                            "Connection pool created but test query failed: {}. Retrying...",
                            e
                        );
                        last_error = Some(e);
                    }
...
async fn test_connection_with_retries(pool: &Pool<MySql>, max_retries: u32) -> Result<()> {
    for attempt in 1..=max_retries {
        log::info!(
            "Testing database connection (test attempt {} of {})...",
            attempt,
            max_retries
        );
        match sqlx::query("SELECT 1").fetch_one(pool).await {
            ...
            Err(e) => {
                if attempt < max_retries {
                    log::warn!(
                        "Test query failed on attempt {}: {}. Retrying...",
                        attempt,
                        e
                    );
    ...
    Err(anyhow::anyhow!(
        "Test query failed after {} attempts",
        max_retries
    ))
}
