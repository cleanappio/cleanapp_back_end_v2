...
pub async fn get_location_feed(
    ...
    // Get total count
    let total = match feed_service::get_feed_count(
        &state.pool,
        params.lat,
        params.lon,
        radius,
...
pub async fn get_tag_feed(
    ...
    // Get reports
    let reports = match feed_service::get_tag_feed(&state.pool, tags, limit).await {
        Ok(reports) => reports,
        Err(e) => {
    ...
    let response = TagFeedResponse { reports, count };
