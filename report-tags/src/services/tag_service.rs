...
pub async fn increment_tag_usage(pool: &Pool<MySql>, tag_id: u64) -> Result<()> {
    sqlx::query("UPDATE tags SET usage_count = usage_count + 1, last_used_at = NOW() WHERE id = ?")
        .bind(tag_id)
        .execute(pool)
        .await?;
    Ok(())
...
pub async fn add_tags_to_report(
    ...
    let report_exists: Option<i32> = sqlx::query_scalar("SELECT seq FROM reports WHERE seq = ?")
        .bind(report_seq)
        .fetch_optional(pool)
        .await?;
    ...
    for tag_string in tag_strings {
        ...
        let (canonical, display) = match normalize_tag(&tag_string) {
            Ok((canonical, display)) => {
                log::debug!(
                    "Normalized tag '{}' to canonical: '{}', display: '{}'",
                    tag_string,
                    canonical,
                    display
                );
            ...
            Err(e) => {
                log::error!(
                    "Failed to normalize tag '{}' for report {}: {}",
                    tag_string,
                    report_seq,
                    e
                );
        ...
        let tag_id = match upsert_tag(pool, &canonical, &display).await {
            ...
            Err(e) => {
                log::error!(
                    "Failed to upsert tag '{}' for report {}: {}",
                    canonical,
                    report_seq,
                    e
                );
        ...
        match sqlx::query("INSERT IGNORE INTO report_tags (report_seq, tag_id) VALUES (?, ?)")
            .bind(report_seq)
            .bind(tag_id)
            .execute(pool)
            .await
        {
            ...
            Err(e) => {
                log::error!(
                    "Failed to insert tag {} (canonical: '{}') into report_tags for report {}: {}",
                    tag_id,
                    canonical,
                    report_seq,
                    e
        ...
        log::debug!(
            "Successfully added tag '{}' to report {}",
            canonical,
            report_seq
        );
    ...
    log::info!(
        "Successfully added {} tags to report {}: {:?}",
        added_tags.len(),
        report_seq,
        added_tags
    );
...
pub async fn follow_tag(
    ...
    // Check follow count
    let count: i64 = sqlx::query_scalar("SELECT COUNT(*) FROM user_tag_follows WHERE user_id = ?")
        .bind(user_id)
        .fetch_one(pool)
        .await?;
    ...
    // Get tag ID
    let tag_id: u64 = sqlx::query_scalar("SELECT id FROM tags WHERE canonical_name = ?")
        .bind(tag_canonical)
        .fetch_one(pool)
        .await?;

    // Insert follow (ignore if exists)
    sqlx::query("INSERT IGNORE INTO user_tag_follows (user_id, tag_id) VALUES (?, ?)")
        .bind(user_id)
        .bind(tag_id)
        .execute(pool)
        .await?;
...
pub async fn unfollow_tag(pool: &Pool<MySql>, user_id: &str, tag_id: u64) -> Result<bool> {
    let result = sqlx::query("DELETE FROM user_tag_follows WHERE user_id = ? AND tag_id = ?")
        .bind(user_id)
        .bind(tag_id)
        .execute(pool)
        .await?;
...
pub async fn get_user_follows(
    pool: &Pool<MySql>,
    user_id: &str,
) -> Result<Vec<crate::models::TagWithFollow>> {
    let rows = sqlx::query(
