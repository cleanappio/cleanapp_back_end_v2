...
struct Args {
    ...
    #[arg(
        long,
        env = "RABBITMQ_TWITTER_REPLY_QUEUE",
        default_value = "twitter-reply"
    )]
    queue: String,
    #[arg(
        long,
        env = "RABBITMQ_TWITTER_REPLY_ROUTING_KEY",
        default_value = "twitter.reply"
    )]
    routing_key: String,

    // Twitter API
    #[arg(long, env = "TWITTER_USER_BEARER_TOKEN")]
    twitter_user_bearer_token: Option<String>,
    // OAuth 1.0a user-context (preferred env names)
    #[arg(long, env = "TWITTER_API_KEY")]
    twitter_api_key: Option<String>,
    #[arg(long, env = "TWITTER_API_SECRET")]
    twitter_api_secret: Option<String>,
    #[arg(long, env = "TWITTER_OAUTH1_ACCESS_TOKEN")]
    twitter_oauth1_access_token: Option<String>,
    #[arg(long, env = "TWITTER_OAUTH1_ACCESS_SECRET")]
    twitter_oauth1_access_secret: Option<String>,

    // CleanApp URL and reply text
    #[arg(long, env = "CLEANAPP_BASE_URL", default_value = "https://cleanapp.io")]
    cleanapp_base_url: String,
    #[arg(
        long,
        env = "TWITTER_REPLY_TEMPLATE",
        default_value = "The relevant cleanapp report was created by your mention: {link} #cleanapped"
    )]
...
impl ReplyCallback {
    ...
    async fn post_reply(&self, in_reply_to_tweet_id: &str, text: &str) -> Result<Option<i64>> {
        ...
        let request_builder = if let Some(ref creds) = self.oauth1 {
            ...
            anyhow::bail!(
                "twitter auth not configured: provide OAuth1 creds or TWITTER_USER_BEARER_TOKEN"
            );
        };
        ...
        let res: std::result::Result<(), ProcessingError> = block_on(async move {
            ...
            match this.post_reply(&evt.tweet_id, &text).await {
                ...
                Err(e) => {
                    ...
                    if let Err(e2) = this
                        .record_attempt(evt.seq, &evt.tweet_id, &evt.classification, None)
                        .await
                    {
...
async fn main() -> Result<()> {
    ...
    let callback = Arc::new(ReplyCallback {
        ...
        oauth1: match (
            ...
            &args.twitter_api_secret,
            &args.twitter_oauth1_access_token,
            &args.twitter_oauth1_access_secret,
        ) {
            (Some(ck), Some(cs), Some(at), Some(as_))
                if !ck.is_empty() && !cs.is_empty() && !at.is_empty() && !as_.is_empty() =>
            {
                Some(OAuth1Creds {
    ...
    let mut routing_map: HashMap<String, Arc<dyn Callback + Send + Sync + 'static>> =
        HashMap::new();
