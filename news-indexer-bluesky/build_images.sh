...
TAG_STREAM="${REPO_PATH}/bluesky-now:${BUILD_VERSION}"
TAG_REPORT_ANALYZER="${REPO_PATH}/cleanapp-report-analyzer-image:${BUILD_VERSION}"
...
gcloud builds submit \
  ...
  --substitutions=_TAG_INDEXER=${TAG_INDEXER},_TAG_ANALYZER=${TAG_ANALYZER},_TAG_SUBMITTER=${TAG_SUBMITTER},_TAG_STREAM=${TAG_STREAM},_TAG_REPORT_ANALYZER=${TAG_REPORT_ANALYZER} \
  .
...
gcloud artifacts docker tags add ${TAG_STREAM} ${REPO_PATH}/bluesky-now:${OPT}
gcloud artifacts docker tags add ${TAG_REPORT_ANALYZER} ${REPO_PATH}/cleanapp-report-analyzer-image:${OPT}
