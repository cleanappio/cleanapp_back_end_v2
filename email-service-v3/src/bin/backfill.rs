...
async fn main() -> Result<()> {
    ...
    tracing::info!(
        "Found {} brands in report_analysis for backfill",
        brand_to_emails.len()
    );

    for (brand_name, emails) in brand_to_emails {
        let display = brand_to_display
            .get(&brand_name)
            .cloned()
            .unwrap_or_else(|| prettify_brand_name(&brand_name));
        db::upsert_brand(
            &mut conn,
            &Brand {
